...
func cursor_above_position(font font_info, expand position vec2) (cursor font_cursor)
{
    var cursor font_cursor;
    cursor.x = position.x cast(s32);
    cursor.y = position.y cast(s32) + font.bottom_margin;
    cursor.line_x = cursor.x;
...
func advance_line(font font_info, cursor font_cursor ref)
{
    cursor.x = cursor.line_x;
    cursor.y = cursor.y - font.line_spacing;    
}
...
func advance(iterator font_text_iterator ref) (glyph_index u32, x_advance s32)
{
    ...
    if code is "\n"[0]
    {        
        advance_line(font, iterator.cursor ref);
        iterator.previous_index = iterator.space_index;
        return -1 cast(u32), 0;
    }
